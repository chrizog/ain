...
public:
    ...
    Res operator()(const CPoolSwapMessageV2& obj) const {
        ...
        if (height >= static_cast<uint32_t>(Params().GetConsensus().GreatWorldHeight)) {
            if (obj.poolIDs.size() == 1 || obj.poolIDs.size() > 3) {
                return Res::Err(strprintf("Unexpected number of pool IDs, only 2 or 3 allowed, %d provided", obj.poolIDs.size()));
            }
        }
...
Res CPoolSwap::ExecuteSwap(CCustomCSView& view, std::vector<DCT_ID> poolIDs) {
    ...
    for (size_t i{0}; i < poolIDs.size(); ++i) {
        ...
        if (height >= static_cast<uint32_t>(Params().GetConsensus().GreatWorldHeight) && lastSwap) {
